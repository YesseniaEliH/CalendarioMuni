...
// partiendo de que ya tenemos creado un objecto EventData previamente utilizamos el contexto
	...
	public static function getEvery(){
			$sql = "select (select color_fondo from category WHERE category.id=event.category_id) as color_fondo, (select color_text from category WHERE category.id=event.category_id) as color_texto, id,title, description,date_at,time_at,created_at,category_id,user_id,project_id  from ".self::$tablename;
			$query = Executor::doit($sql);
